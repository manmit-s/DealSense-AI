...
from schemas.dashboard import DashboardOverview, FunnelStage, AgentActivityResponse
...
async def get_overview(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # 2. Deals at risk (Health < 65)
    risk_stmt = select(func.count(Deal.id)).where(
        and_(Deal.org_id == org_id, Deal.stage.notin_(["Closed Lost", "Closed Won"]), Deal.health_score < 65)
    )
    ...
    # 4. Accounts at risk (Churn Score >= 35)
    account_stmt = select(func.count(Account.id)).where(
        and_(Account.org_id == org_id, Account.churn_score >= 35)
    )
